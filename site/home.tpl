			...
			<td>
				<div class="progress">
					...
					role="progressbar" aria-valuenow="{$Hero->CurrentHP}" aria-valuemin="0" aria-valuemax="{$Hero->MaxHP}" style="width:{$Hero->CurrentHP/$Hero->MaxHP*100}%" id="healthBar-{$Hero->ID}">
						<span><div id='currentHPSpan-{$Hero->ID}'>{$Hero->CurrentHP}</div>HP/{$Hero->MaxHP}HP{if $Hero->isAlive() == false} <a href='delete.php?ID={$Hero->ID}'>Remove</a>{elseif $Hero->CurrentHP <= 0} <a href='revive.php?ID={$Hero->ID}'>Revive</a>{/if}</span>
					</div>
